...
import { Mail, Phone, MapPin, Clock, ShieldCheck, Zap, Factory, Building2, Wrench, Sun, Headphones, Menu, X, ChevronRight } from 'lucide-react';
...
const facebookUrl = 'https://www.facebook.com/urbinstal.sa/';
const linkedinUrl = 'https://www.linkedin.com/company/urbinstal-s-a-/';
...
function FacebookIcon({ size = 20 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path fill="currentColor" d="M14 8.5V6.75c0-.8.18-1.25 1.32-1.25H17V2.6c-.82-.08-1.64-.12-2.46-.12-2.44 0-4.1 1.49-4.1 4.22v1.8H7.7v3.25h2.74V22h3.32V11.75h2.78L17 8.5h-3Z" />
    </svg>
  );
}
...
function LinkedinIcon({ size = 20 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      <path fill="currentColor" d="M6.94 8.75H3.56V21h3.38V8.75ZM5.25 3a1.96 1.96 0 1 0 0 3.92A1.96 1.96 0 0 0 5.25 3Zm15.2 11.37c0-3.28-1.75-5.39-4.59-5.39-1.57 0-2.63.86-3.07 1.68h-.05V8.75H9.5V21h3.37v-6.06c0-1.6.3-3.15 2.28-3.15 1.95 0 1.98 1.83 1.98 3.25V21h3.36v-6.63Z" />
    </svg>
  );
}
...
function Obras() {
  ...
  return <section id="obras" className="section dark-section"><Container><div className="title reveal"><div className="section-kicker">Obras</div><h2>Portfólio técnico por área de intervenção.</h2><p>Projetos em urbanizações, redes exteriores, indústria, instalações elétricas, manutenção e energias renováveis.</p></div><Carousel className="works-slider reveal" indicators>{works.map(w=><Carousel.Item key={w.title}><Row className="align-items-center g-4"><Col lg={7}><button className="work-image" onClick={()=>{setCurrent(w);setShow(true)}}><img src={w.img} alt={w.title} loading="lazy" /></button></Col><Col lg={5}><h3>{w.title}</h3><p>{w.text}</p><Button as="a" href={quoteMail} target="_blank" rel="noreferrer" className="btn-red">Pedir orçamento</Button></Col></Row></Carousel.Item>)}</Carousel></Container><Modal show={show} onHide={()=>setShow(false)} centered size="xl"><Modal.Body className="p-0 lightbox"><button className="lightbox-close" onClick={()=>setShow(false)}>×</button><img src={current.img} alt={current.title}/><div><h3>{current.title}</h3><p>{current.text}</p></div></Modal.Body></Modal></section>;
}


function PorqueEscolher() {
  const reasons = [
    { icon: ShieldCheck, title: 'Rigor Técnico', text: 'Execução com atenção ao detalhe, normas aplicáveis e qualidade em cada fase.' },
    { icon: Zap, title: 'Segurança', text: 'Intervenções orientadas para prevenção, fiabilidade e continuidade operacional.' },
    { icon: Clock, title: 'Cumprimento', text: 'Planeamento cuidado, comunicação clara e compromisso com prazos assumidos.' },
    { icon: Building2, title: 'Experiência', text: 'Conhecimento prático em infraestruturas, indústria, urbanizações e redes técnicas.' }
  ];
    ...
    <section className="why-section reveal">
      <Container>
        <Row className="align-items-center g-4">
          <Col lg={4}>
            <div className="section-kicker">Porque escolher</div>
            <h2>Uma equipa preparada para projetos exigentes.</h2>
          </Col>
          <Col lg={8}>
            <Row className="g-3">
    ...
    <footer className="site-footer">
      <Container>
        <Row className="g-4 align-items-start">
          <Col lg={4}>
            <img src="/assets/logo.png" alt="Urbinstal" className="footer-logo" />
            <p>Energia, infraestruturas, manutenção e soluções técnicas para projetos exigentes.</p>
            <div className="footer-socials">
              <a href={whatsappUrl} target="_blank" rel="noreferrer" aria-label="WhatsApp"><WhatsAppBrandIcon size={20}/></a>
              <a href={telUrl} aria-label="Ligar agora"><PhoneSolidIcon size={19}/></a>
              <a href={facebookUrl} target="_blank" rel="noreferrer" aria-label="Facebook"><FacebookIcon size={20}/></a>
              <a href={linkedinUrl} target="_blank" rel="noreferrer" aria-label="LinkedIn"><LinkedinIcon size={20}/></a>
            </div>
          </Col>
          <Col lg={4}>
            <h4>Contactos</h4>
            <p>+351 918 834 769</p>
            <p>admin@urbinstal.pt</p>
            <p>comercial@urbinstal.pt</p>
            <p>tecnico@urbinstal.pt</p>
          </Col>
          <Col lg={4}>
            <h4>Navegação</h4>
        ...
        <div className="footer-bottom">© 2025 Urbinstal. Todos os direitos reservados.</div>
      </Container>
    </footer>
  );
}

function App(){useReveal(); return <><Header/><Hero/><Counters/><Empresa/><Servicos/><PorqueEscolher/><Obras/><Recrutamento/><Contactos/><Footer/></>}
